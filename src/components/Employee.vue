...
<template>
    <div :class="`flex flex-col max-w-64 p-4 gap-y-1 bg-white border border-gray-200 rounded-lg shadow hover:cursor-pointer ${background_color}`">
        <span class="flex font-medium text-center mx-auto">{{employee.current["Name"]}}</span>
        <span class="flex text-center mx-auto text-gray-600 {{background_text}}">{{employee.current["Job Title"]}}</span>
        <div class="flex flex-row flex-wrap gap-1 justify-center text-sm">
            <span :class="`flex text-center px-1.5 rounded-2xl border-2 border-gray-800 border-opacity-60 ${background_text}`">{{employee.current["Department"]}}</span>
            <span :class="`flex text-center px-1.5 rounded-2xl border-2 border-gray-800 border-opacity-60 ${background_text}`">Level {{employee.current["level"]}}</span>
            <div :class="`flex flex-row px-1.5 gap-x-1 rounded-2xl border-2 border-gray-800 border-opacity-60 ${background_text}`">
                ...
                <span class="flex text-center">{{employee.current["Location"]}}</span>
            </div>
        </div>
